...
//
// Second vertical slice of M1: replaces the markdown-driven
// `/fluxmirror:report-today` (Claude/Qwen) and `/fluxmirror:today`
// (Gemini) slash commands with a single `fluxmirror today` subcommand
// that emits a finished, human-readable report on stdout. The slash
// command files shrink to a 2-line wrapper that calls this binary and
// forwards its output verbatim.
//
...
pub fn run(args: TodayArgs) -> ExitCode {
    ...
    let day = match collect_day(&conn, tz, start_utc, end_utc) {
        Ok(d) => d,
        Err(e) => return err_exit2(format!("fluxmirror today: {e}")),
    };

    let lp = pack(&args.lang);
    let report = render_human(lp, &args.tz, target_date, &day);
    print!("{}", report);
    ExitCode::SUCCESS
}
...
pub(crate) fn collect_day(
    ...
    // Pass 1 — every agent_events row in the window.
    let mut stmt = conn
        .prepare(
    ...
    for row in rows {
        ...
        if is_write(tool_str) && !detail.is_empty() {
            *day.files_edited.entry((detail.clone(), tool.clone())).or_default() += 1;
            day.writes_total += 1;
...
fn render_human(lp: &LangPack, tz_name: &str, date: NaiveDate, day: &DayStats) -> String {
    ...
    // Title: "# Today's Work (YYYY-MM-DD <tz>)".
    out.push_str(&format!(
        "# {} ({}, {})\n\n",
        lp.today_title,
        date.format("%Y-%m-%d"),
    ...
    // Section 1: Activity stats.
    render_activity(&mut out, lp, day);
    // Section 2: Files written or edited.
    render_files_edited(&mut out, lp, day);
    // Section 3: Files only read.
    ...
    // Section 6: MCP traffic methods (skip if empty).
    render_mcp(&mut out, lp, day);
    // Section 7: Tool mix.
    render_tool_mix(&mut out, lp, day);
    // Section 8: Hour distribution.
...
fn render_files_edited(out: &mut String, lp: &LangPack, day: &DayStats) {
    ...
    // Sort by count desc, then path asc, then tool asc — fully stable.
    rows.sort_by(|a, b| {
        b.1.cmp(a.1)
...
fn render_shells(out: &mut String, lp: &LangPack, day: &DayStats) {
    ...
    out.push_str("|---|---|\n");
    for s in &day.shells {
        // Escape pipe chars in detail to keep markdown table well-formed.
...
fn compute_insights(lp: &LangPack, day: &DayStats) -> Vec<String> {
    ...
    // Rule 2: edit-to-read ratio. Format with two decimals.
    ...
    // Rule 3: multi-project — count cwds with ≥ MULTI_PROJECT_CWD_MIN_CALLS calls.
    let busy_cwds = day
        .cwds
        .values()
...
fn busiest_hour(hours: &[u64; 24]) -> Option<(usize, u64)> {
...
// Unused for now but exercised by future slices that share the helper.
#[allow(dead_code)]
fn local_iso(tz: Tz, dt: DateTime<Utc>) -> String {
...
mod tests {
    ...
    /// Build a fixture DB and insert the supplied rows. Each row is
    /// `(ts, agent, tool, session, detail, cwd)`.
    ...
    fn collect_day_aggregates_basic_counts() {
        ...
        let day = collect_day(&conn, tz, start, end).unwrap();
    ...
    fn render_human_includes_title_and_activity_table() {
        ...
        assert!(s.contains("Most productive hour"), "missing busiest hour:\n{s}");
    }

    #[test]
    fn render_human_korean_translates_title_and_no_data() {
        let lp = pack("korean");
        let day = DayStats {
            total_events: 1,
            ..Default::default()
        ...
        assert!(s.contains("오늘 활동 적음."), "ko empty line:\n{s}");
    }
    ...
    fn compute_insights_emits_busiest_and_ratio_when_present() {
        ...
        let bullets = compute_insights(lp, &day);
        // Two bullets — busiest hour and ratio.
    ...
    fn shell_table_truncates_long_detail() {
        ...
        // Long command was truncated, so the rendered shell row is at
        // most SHELL_DETAIL_MAX_CHARS x's plus markdown framing.
