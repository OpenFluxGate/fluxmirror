...
use fluxmirror_core::report::dto::{DailyNarrative, NarrativeSource};
...
///
/// `narrative` is the Phase 4 M-A2 daily paragraph. Pass `None` when
/// the surface doesn't carry an AI overlay (e.g. yesterday's HTML
/// card today, or the test fixtures); a heuristic-source paragraph
/// is rendered with an `(estimate)` footnote.
pub(crate) fn render_day_card(
    ...
    narrative: Option<&DailyNarrative>,
) -> String {
...
fn render_day_document(
    ...
    if let Some(n) = narrative {
        push_narrative_section(&mut out, n);
    }
...
/// Render the Phase 4 M-A2 narrative paragraph at the top of the
/// summary block. Heuristic-source paragraphs carry an `(estimate)`
/// footnote so the user knows the AI surface declined.
fn push_narrative_section(out: &mut String, narrative: &DailyNarrative) {
    ...
    out.push_str(&format!(
        "<p>{}</p>\n",
        html_escape(&narrative.paragraph)
    ));
    if matches!(narrative.source, NarrativeSource::Heuristic) {
        out.push_str("<p class=\"narrative-footnote\"><em>(estimate)</em></p>\n");
    }
    out.push_str("</section>\n");
}
...
.narrative p { margin: 0; font-size: 0.95rem; color: #1f2937; }
.narrative-footnote { margin-top: 6px !important; font-size: 0.8rem; color: #6b7280; }
"#;
...
mod tests {
    ...
    #[test]
    fn render_today_card_with_narrative_includes_section_and_estimate_footnote() {
        use fluxmirror_core::report::dto::{DailyNarrative, NarrativeSource};
        ...
        row.sessions.insert("s1".into());
        day.agents.insert("claude-code".into(), row);
        let narrative = DailyNarrative {
            paragraph: "A short heuristic paragraph for the test.".into(),
        ...
        assert!(s.contains("A short heuristic paragraph"));
        assert!(s.contains("(estimate)"));
    }

    #[test]
    fn render_today_card_with_llm_narrative_omits_estimate_footnote() {
        use fluxmirror_core::report::dto::{DailyNarrative, NarrativeSource};
        ...
        day.agents.insert("claude-code".into(), row);
        let narrative = DailyNarrative {
            paragraph: "An LLM-shaped paragraph.".into(),
        ...
        assert!(s.contains("An LLM-shaped paragraph"));
        assert!(!s.contains("(estimate)"));
    }
