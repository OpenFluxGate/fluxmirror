//! Studio JSON API.
//!
//! Phase 3 M2 surface: `/api/today`, `/api/week`, `/api/now`. Every
//! handler resolves a window via `fluxmirror-core::report::data` and
//! returns the canonical DTO as JSON. The studio frontend is the
//! primary consumer; downstream tooling can roundtrip the same shape
//! since the DTOs derive `serde::Deserialize` too.
