...
echo ""
echo "=== Tool mix ==="
fluxmirror sqlite --db "$DB" "SELECT tool, COUNT(*) FROM agent_events WHERE ts >= '$START_UTC' AND ts < '$END_UTC' GROUP BY tool ORDER BY COUNT(*) DESC"

echo ""
echo "=== Hour distribution (local) ==="
fluxmirror histogram --db "$DB" --tz "$USER_TZ" --start "$START_UTC" --end "$END_UTC"

# (streak / first-touch analysis is now done in the model layer; see Step 2)
```
...
Multi-project rule: only mention if ≥ 2 cwds each had ≥ 5 calls today.
